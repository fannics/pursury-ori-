        ...
        <div class="col-xs-12">
            <div class="panel">
                <div class="panel-body">
                    <div class="row">
                        <div class="col-xs-6">
                            <h2>{{ trans('brands.index.brands') }}</h2>
                            <p>
                                {{ trans('brands.index.summary') }}
                            </p>
                        </div>
                        <div class="col-xs-6 text-right">
                            <a class="btn btn-primary" target="_blank" href="{{ route('admin.brands.export') }}">{{ trans('category.index.export') }}</a>
                            <a class="btn btn-success" href="{{ route('admin.brands.importView') }}">{{ trans('category.index.import') }}</a>
                            <a class="btn btn-info" target="_blank" href="{{ route('admin.brands.export.blank') }}">{{ trans('category.index.blank_template') }}</a>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            var createProductsTable = function(){
                ...
                table.dataTable({
                    ajax: {
                        url: '{{route('admin.brands.index.dataTable')}}',
                        method : 'POST'
                    },
                    language: {
                    ...
                    buttons: [
                        {
                            ...
                            action: function(e, dt, node, config){
                                ...
                                bootbox.confirm("{{ trans('brands.index.confirm_categories_delete') }}", function(res){
                                    if (res){
                                        $.post('{{route('admin.brands.destroy')}}', {action: 'remove', ids: ids})
                        ...
                        },
                        {
                            ...
                            action: function(e, dt, node, config){
                                ...
                                bootbox.confirm("{{ trans('brands.index.confirm_categories_visibility') }}", function(res){
                                    if (res){
                                        $.post('{{route('admin.brands.modify.visibility')}}', {action: 'toggle', ids: ids})
                                                .success(function(res){
                                                    dt.ajax.reload(null, true);
                                                    bootbox.alert(res.message);
                                                })
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            render: function( data, type, full, meta){
                                ...
                                return '<a href="' + window.app_prefix + '/admin/brands/edit/' + full.id + '"><i class="fa fa-edit fa-lg"></i></a>' +
                                        '<a href="#" class="global-action" data-action="remove" data-otype="Brand" data-oid="' + full.id + '" data-confirm="{{ trans('brands.index.confirm_delete_single') }}"  ><i class="fa fa-trash fa-lg"></i></a>';
                            }
                        }
                    ]
                });
