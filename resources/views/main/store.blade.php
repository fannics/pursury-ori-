...
@section('metadata')
    ...
    <!-- Open Graph data -->
    <meta property="og:title" content="{{ settings('app.app_title').' - '.$store->meta_title }}" />
    <meta property="og:type" content="website" />
    <meta property="og:image" content="{{ asset(settings('app.route_prefix').'/images/watermarksincom_resized.png') }}" />
    <meta property="og:description" content="{{ $store->short_description }}" />
...
@section('title', settings('app.app_title').' - '.$store->meta_title)

@section('main_content')
    <div class="row">
        <div class="col-xs-12 col-sm-12 {{ $products->currentPage() == 1 ? ' col-md-8 ' : ' col-md-10 product-list-without-description ' }} col-md-push-2">
            <div class="row">
        ...
        <!-- END PRODUCT LIST -->
        <div class="col-xs-12 col-sm-12 col-md-2 sidebar {{ $products->currentPage() == 1  ? ' col-md-pull-8 ' : ' col-md-pull-10 ' }} visible-md visible-lg ">
        ...
        @if ($products->currentPage() == 1  )
            <div class="col-xs-12 col-sm-12 col-md-2">
                <div class="category-description-wrapper">
                    @if(file_exists(public_path('images/stores/logo') . '/' . $store->logo) && (!is_null($store->logo)))
                        <img style="width:50px; height: 50px" src="{{asset('images/stores/logo') . '/'. $store->logo}}">
<br>
                    @endif
